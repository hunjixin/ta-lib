...
///
/// Arguments:
/// - `df`: pointer to a `DataFrame(f64)` containing the OHLCV data
///         The DataFrame must include columns named: `"high"`, `"low"`, `"close"`, and `"volume"`
...
pub fn Mfi(df: *const DataFrame(f64), inTimePeriod: usize, allocator: std.mem.Allocator) ![]f64 {
...
test "Mfi calculation works correctly" {
    ...
    try df.addColumnWithData("high", &[_]f64{ 13.27, 15.84, 11.46, 16.92, 14.15, 10.58, 18.33, 12.71, 17.49, 9.94, 19.02, 11.88, 13.63, 14.91, 16.47, 12.39, 15.02, 10.73, 17.76, 13.05 });
    try df.addColumnWithData("low", &[_]f64{ 11.12, 13.91, 10.08, 15.44, 12.77, 9.86, 16.50, 11.62, 15.88, 8.94, 17.11, 10.55, 12.41, 13.59, 14.89, 10.94, 13.31, 9.21, 15.61, 11.82 });
    try df.addColumnWithData("close", &[_]f64{ 12.20, 14.65, 10.90, 16.30, 13.60, 10.15, 17.40, 12.10, 16.73, 9.40, 18.15, 11.11, 13.02, 14.20, 15.69, 11.60, 14.30, 10.01, 16.80, 12.51 });
    try df.addColumnWithData("volume", &[_]f64{ 1100.0, 980.0, 1250.0, 1490.0, 1320.0, 1180.0, 1600.0, 1410.0, 1530.0, 1010.0, 1650.0, 1200.0, 1370.0, 1450.0, 1590.0, 1110.0, 1420.0, 990.0, 1510.0, 1300.0 });

    const mfi = try Mfi(&df, 8, gpa);
    defer gpa.free(mfi);
